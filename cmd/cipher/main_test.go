...
// TestInfoCmd verifies that `cipher info` returns JSON metadata for a
// sops-encrypted file.
func TestInfoCmd(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), id.Recipient().String()) {
		t.Errorf("info output missing recipient, got:\n%s", stdout.String())
	}
}

// TestRecipientsListCmd verifies that `cipher recipients list` returns
// the encrypted file's recipients.
func TestRecipientsListCmd(t *testing.T) {
	id, err := age.GenerateX25519Identity()
	if err != nil {
		t.Fatalf("identity: %v", err)
	}
	...
	cmd := newRecipientsListCmd()
	cmd.SetArgs([]string{target})
	cmd.SetContext(context.Background())
	var stdout bytes.Buffer
	cmd.SetOut(&stdout)
	if err := cmd.Execute(); err != nil {
		t.Fatalf("recipients list: %v", err)
	}
	if !strings.Contains(stdout.String(), id.Recipient().String()) {
		t.Errorf("list output missing recipient, got:\n%s", stdout.String())
	}
}

// TestConfigCheckCmd verifies that `cipher config check` flags broken
// regexes and unknown identifiers in a .sops.yaml.
func TestConfigCheckCmd(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "path_regex") {
		t.Errorf("stderr missing path_regex problem: %q", stderr.String())
	}
	if !strings.Contains(stderr.String(), "kms") {
		t.Errorf("stderr missing kms problem: %q", stderr.String())
	}
}
