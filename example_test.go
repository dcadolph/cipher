...
func ExampleEncoder() {
	...
	enc := cipher.NewEncoderWith(
		cipherage.NewProvider(recipient),
...
func ExampleRouter() {
	prodProvider := cipherage.NewProvider("age1prod...")
	devProvider := cipherage.NewProvider("age1dev...")
...
func ExampleAddRecipient() {
	newRecipient := cipherage.NewProvider("age1bob...")
...
func ExampleRemoveRecipient() {
	pruned, err := cipher.RemoveRecipient(
		context.Background(), "secrets.yaml", []byte("..."), "age1bob...",
	)
	if err != nil {
		log.Fatal(err)
	}
